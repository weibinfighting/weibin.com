...
Pb = function(params,sigmaknown){
  if(sigmaknown){
    ...
    Pb = abs(pnorm((c+params[3]-params[4])*sqrt(params[1])/params[5])-pnorm((-c+params[3]-params[4])*sqrt(params[1])/params[5])-params[6])
    return(Pb)
  }else{
    c = params[5]*qt(1-params[2]/2,0,1)/sqrt(params[1],n-1)
    Pb = abs(pt((c+params[3]-params[4])*sqrt(params[1])/params[5],n-1)-pt((-c+params[3]-params[4])*sqrt(params[1])/params[5])-params[6],n-1)
    return(Pb)
  }
}
...
secerror = function(x,y=NULL,alpha=0.05,beta=0.05,mu=0,mu1=NULL,mu2=NULL,sigma=NULL,sigma1=NULL,sigma2=NULL,alternative=c("two-side","less","greater"),varequal=TRUE){
  n = length(x)
  ifelse(is.null(mu1),mu1<-mean(x),mu1<-mu1)
  if(is.null(y)){if(is.null(sigma)){              #don't know sigma  result = secerror(x=rnorm(25,980,65),mu=1000,mu1 = 980,alpha = 0.05,beta = 0.1,sigma1 = 65,alternative = "less")
    sigma = sigma1
    ...
    if(is.null(sigma1)|is.null(sigma2)){         #double sample sigma don't know
      ...
      P = pt((c+mu2-mu1)/sw/sqrt(1/m+1/n),m+n-2)-pt((-c+mu2-mu1)/sw/sqrt(1/m+1/n),m+n-2)
      result = cat("犯第二类错误的概率为:P=",P,"\n")
      return(result)
    }
    ...
    return(result)
      ...
      if(alternative=="two-side"){              #H_1:mu1<>mu2
        ...
        P = pnorm((c-mu1+mu2)/sqrt(sigma1^2/m+sigma^2/n))-pnorm((-c-mu1+mu2)/sqrt(sigma1^2/m+sigma^2/n))
        result = cat("犯第二类错误的概率为:P=",P,"\n")
        return(result)
      }
      if(alternative=="less"){                    #H_1:mu1<mu2
...
var_secerror = function(x,y=NULL,alpha=0.05,beta=0.1,sigma=NULL,sigma1=NULL,sigma2=NULL,alternative=c("two-side","less","greater")){
  if(is.null(sigma)){
  ...
  if(is.null(y)){                               #single sample
    ...
    if(alternative=="two-side"){
      ...
      Pb = function(params){
        ...
        Pb = abs(pchisq(params[1]*c_2/params[3]^2,params[1]-1)-pchisq(params[1]*c_1/params[3]^2,params[1]-1)-params[5])
        return(Pb)
      }
    ...
    if(alternative=="two-side"){
      ...
      P = pf((n-1)*m*sigma2^2/((m-1)*n*sigma1^2)*c_12_c_22,m-1,n-1)-pf((n-1)*m*sigma2^2/((m-1)*n*sigma1^2)*c_11_c_21,m-1,n-1)
      result = cat("犯第二类错误的概率为:P=",P,"\n")
      return(result)
    }
    if(alternative=="less"){
